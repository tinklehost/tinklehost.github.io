...
k.ENVIRONMENT_IS_PTHREAD || k.$ww || function(a) {
    ...
    "remote_package_size": 25770085,
    "package_uuid": "31e17389-d58d-47d8-b215-3eb1b87c7686"
});
...
var he = (a, b) => {
    // Guard: if we didn't get a real canvas, just bail out
    if (!a || typeof a.getContext !== "function") {
        return 0;
    }

    // Save original getContext only once
    if (!a.dk) {
        // bind so "this" is the canvas
        a.dk = a.getContext.bind(a);
        a.getContext = function(d, e) {
            var ctx = a.dk(d, e);
            // If no context, just return it (null)
            if (!ctx) return ctx;

            // Keep the original intent: only allow WebGL contexts
    ...
    // Request webgl2 if Pj > 1, otherwise webgl
    var c = (b && b.Pj > 1)
